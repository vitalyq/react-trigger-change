...
module.exports = function reactTriggerChange(node) {
  ...
  function safeDeleteProperty(elem, prop) {
  ...
  function changeRangeValue(elem) {
    var initMin = elem.min;
    var initMax = elem.max;
    var initStep = elem.step;
    var initVal = Number(elem.value);

    elem.min = initVal;
    elem.max = initVal + 1;
    elem.step = 1;
    elem.value = initVal + 1;
    safeDeleteProperty(elem, 'value');
    elem.min = initMin;
    elem.max = initMax;
    elem.step = initStep;
    elem.value = initVal;
  ...
  if (nodeName === 'select' ||
    ...
    safeDeleteProperty(node, 'checked');
    node.checked = initialChecked;
